...
class Coach():
    ...
    def executeEpisode(self):
        ...
        while True:
            episodeStep += 1
            canonicalBoard = self.game.getCanonicalForm(board, self.curPlayer)  #curPlayer = 1
            temp = int(episodeStep < self.args.tempThreshold)

            pi = self.mcts.getActionProb(board, self.curPlayer, temp=temp)  #curPlayer = 1
            sym = self.game.getSymmetries(canonicalBoard, pi)
            for b,p in sym:
                trainExamples.append([b, self.curPlayer, p, None])  #curPlayer = 1

            action = np.random.choice(len(pi), p=pi)
            board, _ = self.game.getNextState(canonicalBoard, 1, action)           
            board = self.game.getOriginalForm(board, self.curPlayer)
            self.curPlayer = -self.curPlayer           #curPlayer = -1 
            ...
            if r != 0:
                ...
                return [(x[0], x[2], r * ((-1) ** (x[1] != self.curPlayer))) for x in trainExamples]    #important!
    ...
    def learn(self):
        ...
        for i in range(1, self.args.numIters + 1):
            ...
            print(trainExamples[1],"\n")
            print(trainExamples[2],"\n")
            print(trainExamples[3],"\n")
            print(trainExamples[4],"\n")
            print(trainExamples[5],"\n")
            print(trainExamples[-2],"\n")
            print(trainExamples[-1],"\n")
    ...
    def loadTrainExamples(self):
        ...
        if self.args.use_mohex_examples:
